...
def ic_loss(model, x, t):
    ...
    u = model(x, t0)
    return torch.mean((u + torch.sin(np.pi * x))**2)    # u(x, 0) = -sin(pi*x)


def bc_left_loss(model, x, t):
    t.requires_grad = True
...
if __name__ == "__main__":
    ...
    targets = {}
    # targets["ic"] = [x, torch.full_like(x, 0)], ic(x)
    # targets["left"] = [torch.full_like(t, -1), t], bc_left(t)
    # targets["right"] = [torch.full_like(t, 1), t], bc_right(t)
    ...
    losses = burgers.fit(inputs=(x_train, t_train), n_epochs=n_epochs, scheduler=scheduler)
