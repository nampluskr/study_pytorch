...
class Discriminator(nn.Module):
    """ Discriminator for CIFAR10 (input [N, 3, 32, 32], output [N, 1] + [N, 10]) """
...
if __name__ == "__main__":
    ...
    p.add_argument("--log_dir", type=str, default="log_pt_cifar10_acgan")
    args = p.parse_args()
    ...
    acgan = my.AuxCondGAN(G, D, noise_dim=args.noise_dim)
    acgan.compile(g_optim=torch.optim.Adam(G.parameters(), lr=2e-4, betas=(0.5, 0.999)),
    ...
    hist = my.train(acgan, train_loader, args, sample_inputs=sample_noises)
